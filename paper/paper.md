...
arises the need for Stream Processing.  
...
1) **Maintaining data integrity:** Unlike batch processing, wherein all of the
incoming data in first stored and then divided into batches for processing,
Stream processing systems must have the capability to handle incoming data as
and when it arrives. This leaves room for Data loss. Another factor that comes
into play when dealing Stream data is ensuring data serialization. i.e. systems
must ensure that messages must be processed in the order in which they were
generated by the source. Thus, maintaining data integrity poses a great
challenge for Stream processing systems.  
2) **State management:** Streaming application need to maintain the state and 
offset to keep track of the last message that was processed. While this 
is relatively simple to do, it poses a big challenge when dealing with system 
failure or in the case of vertical scaling since the new version of the 
application may pose compatibility issues.
...
Netflix and eBay for providing real time analytics.
...
database or a file system [https://www.infoworld.com/article/2854894
/application-development/spark-and-storm-for-real-time-computation.html]. Like
Spark, Storm also provides high fault-tolerance, but it does not have a feature
to use the same code for batch and stream processing. Current users of Storm
include Twitter, Groupon, Yahoo and Spotify for its ability to create low
latency distributed systems for streaming.
...
Kinesis Video Streams, Kinesis Data Firehose and Kinesis Data Analytics
[https://docs.aws.amazon.com/streams/latest/dev/introduction.html]. Data streams
reads the data from various input sources in the form of data records. These
data records can be analyzed or stored in whichever way the application demands.
The data streams start ingesting data within a second of when the data is added.
The data then can be sent to any of the built-integrations or can be stored in
various third-party stores such as DynamoDB or Cassandra. One advantage that
Kinesis Streams offers is scaling of the applications, it allows dynamic
...
analytics in a simple visual fashion.
